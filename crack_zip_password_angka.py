#!/usr/bin/env python3
"""
Script untuk crack password ZIP dengan digit angka (0-9)
Password akan dicoba dari 1 digit sampai 9 digit (0 - 999999999)
...
def print_progress_bar(current, total, digits, bar_length=50):
    """
    Menampilkan progress bar untuk setiap panjang digit
    """
    percent = (current / total) * 100
    filled = int(bar_length * current / total)
    bar = '█' * filled + '░' * (bar_length - filled)
    print(f"\r[*] {digits} digit: [{bar}] {percent:.1f}% ({current:,}/{total:,})", end='', flush=True)
...
def crack_with_zipfile(zip_path, min_digits, max_digits):
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zip_file:
            ...
            # Calculate range
            start_num = 0
            end_num = 10 ** max_digits - 1
            total_combinations = end_num + 1
            
            print(f"\n[*] Mencoba password {min_digits}-{max_digits} digit (0 - {end_num:,})")
            ...
            for num in range(start_num, end_num + 1):
                ...
                if tried % 1000 == 0 or tried == total_combinations:
                    current_digits = len(password)
                    print_progress_bar(tried, total_combinations, f"{min_digits}-{max_digits}", bar_length=50)
                ...
                try:
                    ...
                    print(f"[+] Panjang: {len(password)} digit")
...
def crack_with_pyzipper(zip_path, min_digits, max_digits):
...
def crack_zip_password(zip_path, min_digits, max_digits):
    """
    ...
    print(f"[*] Range: {min_digits}-{max_digits} digit")
    print(f"[*] Character set: 0-9 (10 karakter)")
    ...
    if enc_type == 'none':
        ...
        if HAS_PYZIPPER:
            ...
            return crack_with_pyzipper(zip_path, min_digits, max_digits)
        else:
        ...
        return crack_with_zipfile(zip_path, min_digits, max_digits)
...
def main():
    ...
    print("="*60)
    print("ZIP Password Cracker - Numeric Brute Force")
    print("Character Set: 0-9 (10 karakter)")
    ...
    print(f"\n[*] Mode: Otomatis mencoba password dari {MIN_DIGITS} digit sampai {MAX_DIGITS} digit")
    print(f"[*] Character set: 0-9 (10 karakter)")
    ...
    for zip_file in zip_files:
        ...
        password = crack_zip_password(str(zip_file), MIN_DIGITS, MAX_DIGITS)
    ...
    with open(LOG_FILE, 'w', encoding='utf-8') as log:
        log.write("="*60 + "\n")
        log.write("ZIP Password Cracking Results (Numeric)\n")
        log.write(f"Digit Range: {MIN_DIGITS}-{MAX_DIGITS}\n")
        log.write(f"Character Set: 0-9\n")
        log.write("="*60 + "\n\n")
